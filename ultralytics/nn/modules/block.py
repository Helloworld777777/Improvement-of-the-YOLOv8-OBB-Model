...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, SimConv, AKConv
...
class SPPF1(nn.Module):
    def __init__(self, c1, c2, k=5):
        ...
        self.atrous_block1 = nn.Conv2d(c_, c_//2, 3, 1, padding=6, dilation=6)
    ...
    def forward(self, x):
        ...
        return self.cv2(torch.cat((x, y1, y2, self.m(y2), image_features), 1))

class SimSPPF(nn.Module):
    """Simlified SPPF with ReLU activate"""
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        ...
        y = list(self.cv1(x).chunk(2, 1))                                          #这里的y有两个值
        y.extend(m(y[-1]) for m in self.m)
...
class Z2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.25):
        ...
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 4 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(BottleneckZ(self.c, self.c, shortcut, g, k=((1, 1), (1, 1)), e=1.0) for _ in range(n))

    def forward(self, x):
        """Forward pass through C2f layer."""
        y0, y1, y2, y3 = list(self.cv1(x).chunk(4, 1))[0], list(self.cv1(x).chunk(4, 1))[1], list(self.cv1(x).chunk(4, 1))[2], list(self.cv1(x).chunk(4, 1))[3]
        y = list(self.cv1(x).chunk(4, 1))[0:2]
        y.extend(m(y2, y3) for m in self.m)
...
class BottleneckZ(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return x+y + self.cv2(self.cv1(y)) + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x)) + self.cv2(self.cv1(y))
...
class Bottleneck_AKConv(Bottleneck):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        expansion.
        """
        super().__init__(self, c1, c2,)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = AKConv(c1, c_, k[0], 1)
        self.cv2 = AKConv(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2
